...
export default function TabBar({ onStart }: TabBarProps) {
  ...
  const on = (k: string) => cur === k || (cur === 'history' && k === 'stats') || (cur === 'settings' && k === 'home')

  const startWorkout = () => {
    if (!S.active) {
      const r = effectiveRoutine(S, todayISO())
      if (r && r.ex.length && onStart) { onStart(r.id); return }
    }
    nav('/workout')
  }
  const Tab = ({ k, icon, to, label }: { k: string; icon: string; to: string; label: string }) => (
