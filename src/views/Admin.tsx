...
import { auditCat, auditLine, fmtWhen } from '../lib/audit'
...
// last twenty events. Paging follows Library's house style — "Show more", not page numbers.
function AuditCard({ tick }) {
  ...
  const load = (c: string = '', before?: number | string) => api('/api/admin/audit?limit=50&cat=' + c + (before ? '&before=' + before : ''))
