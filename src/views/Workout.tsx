...
import { effectiveRoutine, lastEntryFor, bestWeightFor, buildSets, freestyleConfig, defaultConfig, setsDoneActive, supersetUnits, unitOf, setLabel, modeOf, isBw, isPerSide, sideReps, repStep, EFFORT, effortOf, stepEffort, capEffort, cascadeWeight, insertWarmupRow, removeRowAt, pairAdjacent, unpairSuperset, cleanupSg } from '../lib/history'
import { fmtNum, fmtDate, todayISO, exCount, DAYN } from '../lib/format'
import { beep, vibrate } from '../lib/sound'
import { t, getLang } from '../lib/i18n'
import { api } from '../lib/api'
import { setProgressHighWater, supersetFlowStep } from '../lib/supersetFlow'
...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onAddWarmup, onRemoveSetAt, onStartTimed, onPairPrev, onPairNext }: any) {
  const S = useStore(s => s.S)
  ...
  const storedW = typeof S.exWeights[entry.id] === 'number' ? S.exWeights[entry.id] : (S.exWeights[entry.id]?.w || 0)
  const best = cardio ? 0 : Math.max(bestWeightFor(S, entry.id), storedW)
  ...
  const added = bw && entry.sets.some((s: any) => Number(s.w) > 0)
  ...
  return <>
    ...
    {plan && plan.why && plan.kind !== 'off' && <div className={'progline' + (plan.kind === 'deload' ? ' warn' : '')}>
      ...
      <span>{Array.isArray(plan.why) ? t(plan.why[0], ...(plan.why.slice(1))) : t(plan.why)}</span>
    </div>}
...
function ActiveWorkout() {
  ...
  const startTimed = (idx: number, i: number) => {
    const e = A.entries[idx]
    useUI.getState().startWork(Number(e.sets[i].sec) || 45, exOr(e.id).n, elapsed => {
  ...
  return <div className="narrow">
    ...
    <div style={{ height: 12 }} />
    <div className="row" style={{ flexDirection: getLang() === 'ar' ? 'row-reverse' : 'row' }}>
    ...
    <Button onClick={() => exercisePicker(ex => {
      ...
      exConfigSheet(ex, null, cfg => update(s => {
        ...
        const plan = freestyle ? null : nextPrescription(s, full, s.routines.find((r: any) => r.id === s.active.routineId))
        const sets = buildSets(s, full, freestyle ? { preferLast: true } : undefined)
        s.active.entries.push({ id: ex.id, target: { ...cfg }, plan, sets: freestyle ? sets : applyPrescription(sets, plan) } as any)
