...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'buildersarmy-backup-' + todayISO() + 'on'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not a buildersarmy backup')
      ...
      } catch (e: any) { toast(t('Import failed: {0}', e.message)) }
    }
    rd.readAsText(f)
  }
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host buildersarmy')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
    ...
    <Section title={t('General')} footer={t('Note: switching units only changes the label — logged numbers are not converted.')}>
      <SelectRow
        ...
        value={S.lang || 'en'} onChange={v => update(s => { s.lang = v as any })}
      ...
      <Row icon="scale" iconTint="var(--teal)" title={t('Weight unit')}>
        <Segmented className="seg-inline"
          ...
          value={S.unit} onChange={v => update(s => { s.unit = v as any })} />
      </Row>
    </Section>
    ...
    <Section title={t('During a workout')} footer={wakeOK ? t('The screen stays on while a workout is running, so you don’t have to unlock your phone between sets.') : null}>
      ...
      <Row icon="target" iconTint="var(--purple)" title={t('Effort per set')}>
        ...
        <Segmented className="seg-inline"
          ...
          value={effortOf(S)} onChange={v => update(s => { s.effort = v as any; delete s.showRir })} />
      </Row>
    </Section>
    ...
    <Section title={t('Appearance')} footer={DEMO || MOBILE ? undefined : t('synced with your profile')}>
      <Row icon="moon" iconTint="var(--indigo)" title={t('Theme')}>
        <Segmented
          ...
          onChange={v => update(s => { s.theme = v as any })}
        />
      </Row>
      ...
      <Row icon="figureStrength" iconTint="var(--teal)" title={t('Body diagram')}>
        <Segmented
          ...
          onChange={v => update(s => { s.body = v as any })}
        />
      </Row>
        ...
        <div className="swatches">
          {Object.entries(ACCENTS).map(([k, c]) => (
            <button key={k} className={'swatch' + ((S.accent || 'lime') === k ? ' on' : '')}
              style={{ background: c }} onClick={() => update(s => { s.accent = k as any })} aria-label={k} />
          ))}
        </div>
      </div>
    </Section>
    ...
    <input ref={fileRef} type="file" accept="on,application/json" style={{ display: 'none' }} onChange={doImport} />
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install buildersarmy as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      buildersarmy · {t('free & open source (AGPL v3)')}<br />
      <a href="https://gitlab.com/DuarteSantos8/buildersarmy" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (MIT)<br />
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if buildersarmy is closed.')}>
