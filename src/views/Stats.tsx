...
import { lastBW, streakWeeks, setLabel, modeOf, effortOf, metricModeForEntry, metricRowsForEntry, bestWeightForEntry } from '../lib/history'
...
import { loadOfWorkouts, rankOf, MUSCLE_NAME, musclesOf } from '../lib/muscles'
import { fatigueOf, strengthOf, STRENGTH_FLOOR, LB_TO_KG } from '../lib/recovery'
import { strengthExerciseRowsForMuscle } from '../lib/strength-exercises'
import { fatigueStateOf } from '../lib/recovery-view'
...
// which ones it keeps missing. Shading is relative within the window (lib/muscles).
...
// is computed in RIR (lib/effort) and converted to whichever scale this profile reads.
...
export default function Stats() {
  ...
  const bwPts = S.bodyweight.filter(b => {
    const time = b.t || (b.d ? new Date(b.d).getTime() : (b.date ? new Date(b.date).getTime() : 0))
    return range === 0 || time > now - range * 86400000
  }).map(b => ({
    t: b.t || (b.d ? new Date(b.d).getTime() : (b.date ? new Date(b.date).getTime() : 0)),
    y: Number(b.w ?? b.weight ?? 0),
    d: b.d || b.date
  }))
  const bw30 = S.bodyweight.filter(b => {
    const time = b.t || (b.d ? new Date(b.d).getTime() : (b.date ? new Date(b.date).getTime() : 0))
    return time > now - 30 * 86400000
  })
  const bwDelta30 = bw30.length > 1 ? Number(bw30[bw30.length - 1].w ?? bw30[bw30.length - 1].weight ?? 0) - Number(bw30[0].w ?? bw30[0].weight ?? 0) : null
  const workouts = S.workouts
