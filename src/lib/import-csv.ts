...
export function detectSource(header: string[]) {
...
/**
 * Read an export into workouts buildersarmy understands, WITHOUT touching state — the caller
...
export function parseWorkoutCSV(text, { unit = 'kg' } = {}) {
  ...
  const workouts = dates.map(d => {
    ...
    const entries = [...day.ex.entries()].map(([id, ss]: [string, any[]]) => {
      const conv2 = ss.map(({ u, ...s }) => (s.w !== undefined ? { ...s, w: convRow({ ...s, u }) } : s))
      const mx = Math.max(0, ...conv2.filter((s: any) => !isWarmupRow(s)).map((s: any) => s.w || 0))
    ...
    w.vol = entries.reduce((a: number, e: any) => a + e.sets.reduce((b: number, s: any) => b + (s.w || 0) * (s.r || 0), 0), 0)
    return w
  })
...
export function parseImport(text: string, opts?: any): any {
  const s = String(text)
  if (s.includes('HKQuantityTypeIdentifier') || /^\s*</.test(s)) return parseBodyweight(s, opts)
  const asWorkouts: any = parseWorkoutCSV(s, opts)
  if (!asWorkouts.error) return asWorkouts
  const asWeights: any = parseBodyweight(s, opts)
...
export function mergeImport(S: any, parsed: any) {
  if (parsed.kind === 'bodyweight') {
    const have = new Set(S.bodyweight.map((b: any) => b.d))
    const fresh = parsed.bodyweight.filter((b: any) => !have.has(b.d))
    S.bodyweight = [...S.bodyweight, ...fresh].sort((a: any, b: any) => (a.d < b.d ? -1 : 1))
    return { added: fresh.length, skipped: parsed.bodyweight.length - fresh.length }
  }
  const have = new Set(S.workouts.map((w: any) => w.d))
  const fresh = parsed.workouts.filter((w: any) => !have.has(w.d))
  const used = new Set(fresh.flatMap((w: any) => w.entries.map((e: any) => e.id)))
  const customs = parsed.customEx.filter((c: any) => used.has(c.id) && !EXIDX[c.id])
  S.customEx = [...(S.customEx || []), ...customs]
  S.workouts = [...S.workouts, ...fresh].sort((a: any, b: any) => (a.d < b.d ? -1 : 1))
  // seed the weight suggestions from the newest imported set of each lift
  fresh.forEach((w: any) => w.entries.forEach((e: any) => {
    const mx = Math.max(0, ...e.sets.map((s: any) => s.w || 0), e.topW || 0)
