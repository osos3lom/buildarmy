...
function bodyweightOf(workout: any, opts: any = {}, stampedLoadUnit?: string) {
  ...
  if (stamped !== null) {
    return kgOf(stamped, unitOf(
      ...
      workout,
      stampedLoadUnit ? { unit: stampedLoadUnit } : undefined,
      opts,
    ))
  }
...
function bodyweightTarget(entry: any) {
...
function hasUnitStamp(...records: any[]) {
...
function bodyweightConfigured(ex: any, entry: any, set: any, workout: any, opts: any = {}) {
...
function loadKgFor(ex: any, entry: any, set: any, workout: any, opts: any = {}) {
  ...
  return bodyweightConfigured(ex, entry, set, workout, opts)
    ? bodyweightOf(workout, opts, loadUnit) + addedKg
    : addedKg
}
  ...
  for (const entry of workout?.entries || []) {
    ...
    for (const set of entry.sets || []) {
      ...
      for (const [slug, weight] of Object.entries(weights)) {
        if (Object.prototype.hasOwnProperty.call(MUSCLES_BY_SLUG, slug)) sums[slug] += tonnage * Number(weight)
      }
    }
  }
  return sums
