...
export function exerciseMuscleSnapshot(ex: any) {
 ...
 * near failure.
 */
export const loadOfWorkouts = (workouts: any[], pick?: (s: any) => boolean) =>
  loadOf((workouts || []).flatMap((w: any) =>
    (w.entries || []).map((e: any) => ({ id: e.id, ex: e.exercise || e, sets: (e.sets || []).filter((s: any) => s.done && !isWarmupRow(s) && (!pick || pick(s))).length }))))

/** Load a routine *would* produce, from its planned set counts. */
export const loadOfRoutine = (routine: any) =>
  loadOf((routine?.ex || []).map((c: any) => ({ id: c.id, ex: c, sets: c.sets || 1 })))

/** Load for a workout still in progress — the sets ticked so far. */
export const loadOfActive = (active: any) =>
  loadOf((active?.entries || []).map((e: any) => ({ id: e.id, ex: e.exercise || e, sets: (e.sets || []).filter((s: any) => s.done && !isWarmupRow(s)).length })))
