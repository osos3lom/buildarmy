...
import { EXDB, EXIDX, BODYPARTS, isCardio, isBodyweightEq, allExercises, equipmentOf, smOf } from './lib/exercises'
import { fmtDate, fmtNum, fmtVol, fmtDur, durPart, todayISO, uid, exCount, DAYN, MONTHS_LONG, ACCENTS } from './lib/format'
import { lastEntryFor, bestWeightFor, buildSets, effectiveRoutineId, workoutVolume, setsDone, setsDoneActive, lastBW, supersetUnits, unitOf, setLabel, defaultConfig, cleanupSg, modeOf, effortOf, isBw, isPerSide, sideReps, workSetsDone } from './lib/history'
...
import { exerciseMuscleSnapshot, loadOfWorkouts } from './lib/muscles'
...
import { buildCompletedWorkout } from './lib/finish-workout'
import { isWarmupRow } from './lib/workout-model'

const S = () => useStore.getState().S
const update = (mut: (draft: any) => void, push?: boolean) => useStore.getState().update(mut, push)
const ui = () => useUI.getState()
const toast = (m: string) => ui().toast(m)
const snd = () => S().sound
...
export function bwSheet(opts: any = {}) {
...
export function importFromApp(file: File, onDone?: (result?: any) => void) {
...
export const exConfigSheet = (ex: any, existing?: any, onSave?: (cfg: any) => void, onDelete?: () => void, routine?: any, initial?: any) => ui().openSheet(close => <ExConfig ex={ex} existing={existing} initial={initial} onSave={onSave} onDelete={onDelete} routine={routine} close={close} />)
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'buildersarmy-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own buildersarmy — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function DayOverride({ iso, close }) {
  ...
  const set = (v: string) => {
    ...
    close()
    const rName = (st.routines.find((r: any) => r.id === v) || {}).name || t('routine')
    toast(v === '' ? t('Back to weekly plan') : v === 'rest' ? t('{0} set to rest', fmtDate(iso)) : t('{0} planned for {1}', rName, fmtDate(iso)))
  }
  return <>
...
export const dayAssignSheet = (day: any) => ui().openSheet(close => <DayAssign day={day} close={close} />)
    ...
    {w.entries.map((e: any, i: number) => {
      ...
      return <div key={i} className="row" style={{ marginBottom: 12, alignItems: 'flex-start' }}>
        ...
        <div className="grow"><div className="tt capitalize" style={{ fontWeight: 600 }}>{ex ? ex.n : (e.n || e.id)} {w.prs && w.prs.includes(e.id) && <span className="pr"><Icon name="trophy" />PR</span>}</div>
          <div className="ss">{e.sets.filter((s: any) => s.done).map((s: any) => setLabel(e.id, s, e.target)).join('  ·  ') || t('no sets')}</div></div>
      </div>
    })}
    <Button variant="danger" onClick={() => confirmSheet({ title: t('Delete workout?'), message: t('This removes it from your history for good.'), confirmText: t('Delete'), danger: true, onConfirm: () => { update(s => { s.workouts = s.workouts.filter(x => x.id !== w.id) }); close(); toast(t('Workout deleted')) } })}>{t('Delete workout')}</Button>
  </>
}
export const workoutDetailSheet = (w: any) => ui().openSheet(close => <WorkoutDetail w={w} close={close} />)
...
function Calendar({ start, close }: any) {
  ...
  for (let d = 1; d <= daysIn; d++) {
    ...
    cells.push(<button key={d} className={'cal-d' + (ws ? ' has' : '') + (iso === todayISO() ? ' today' : '')} onClick={() => {
      ...
      close(); ui().openSheet(c2 => <><h3>{fmtDate(iso, true)}</h3><div className="list">{ws.map((w: any) => <WorkoutRow key={w.id} w={w} onClick={() => { c2(); workoutDetailSheet(w) }} />)}</div></>)
...
function TopWeight({ entryIdx, close }) {
  ...
  const maxSet = entry ? Math.max(0, ...entry.sets.filter((s: any) => s.done && !isWarmupRow(s)).map((s: any) => Number(s.w) || 0)) : 0
...
function doFinishWorkout() {
  ...
  A.entries.forEach((e: any) => {
    const mx = Math.max(0, ...e.sets.filter((s: any) => s.done && !isWarmupRow(s)).map((s: any) => Number(s.w) || 0))
  ...
  const w: any = buildCompletedWorkout(A, {
    end: Date.now(),
    prs,
    snapshotFor: (e: any) => EXIDX[e.id]?.custom ? exerciseMuscleSnapshot(EXIDX[e.id]) : null,
  })
  w.vol = workoutVolume(w)
  update(s => {
    w.entries.forEach((e: any) => {
      const mx = Math.max(0, ...e.sets.filter((x: any) => x.done && !isWarmupRow(x)).map((x: any) => Number(x.w) || 0), Number(e.topW) || 0)
